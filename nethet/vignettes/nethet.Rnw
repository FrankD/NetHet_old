...
heterogeneous data consisting of different groups (or components). Often,
we would like to perform a statistical comparison between estimated
networks. In this section we will introduce functions which allow
formal hypothesis testing regarding network differences.

%% show how to use the functions that allow
%% for high-dimensional hypothesis testing; in other words, how to
%% compare reconstructed networks (and regression models) and test
%% whether the networks that arise from a given dataset are significantly
%% different.

\subsection{Differential network}
\label{sec:diffnet}
Consider two datasets $\mathbf{X}_1$ and $\mathbf{X}_2$ collected
under different conditions. We would like to know whether networks
estimated from these two datasets differ in a statistical significant
manner. The function \texttt{diffnet\_multisplit} uses repeated sample
splitting to address this task: the data is randomly split into two
halves, networks are inferred on one half and significance testing
(p-value calculation) is performed on the other half. In order to
prevent a p-value lottery this process is repeated many times and the
resulting p-values are aggregated to obtain a final p-value.

We will use the inverse covariance matrices generated in Section
...
## (both datasets have the same underlying network)
