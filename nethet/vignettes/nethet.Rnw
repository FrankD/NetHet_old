...
The output of \texttt{het_cv_glmnet} and \texttt{mixglasso} can be plotted either
as in network form or as individual edges in the networks. For the network plots,
we use the \texttt{network} package \cite{network}. This is the default plotting
when \texttt{plot} is invoked on an object of class \texttt{nethetclustering},
and produces one global plot showing edges that occur in any group, as well as 
one plot for each group. For this example we will use the networks and 
clustering obtained using \texttt{mixglasso} in the previous section.
...
them among groups. Function \texttt{dotPlot} generates a plot with edges above
...
package to make the plots \cite{ggplot2}.

<<echo=TRUE, include=TRUE>>=

# Plot edges, omitting those with absolute partial correlation < 0.5 in every 
# group.
dotPlot(mixglasso.clustering, p.corrs.thresh=0.5)
@

Finally, we might want to compare the observed values of the nodes linked by
specific edges across groups. Function scatterPlot will generate plots for
a specified list of edges.

<<echo=TRUE, include=TRUE>>=

# Specify edges
node.pairs = rbind(c(1,7), c(5,9),c(7,8))

# Create scatter plots of specified edges
scatterPlot(mixglasso.clustering, data=sim.result$data,
...
\texttt{export_network} can be invoked on the output of \texttt{het_cv_glmnet} 
and \texttt{mixglasso}.
...
the network further, we note that the output of \texttt{export_network} can be
