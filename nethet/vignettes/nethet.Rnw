...
novel and very general methodology for high-dimensioal two-sample
...
in the high-dimensional regression model.

\subsection{Differential network}
Let us consider datasets generated from GGMs $G_1$ and $G_2$
respectively. We would like to know whether networks inferred form
...
\item Based on the \emph{out}-sample, a p-value is computed which
  compares the neworks obtained in step 2 (``cleaning step'').
...
previsously generated in Section \ref{sec:simulation}.

<<eval=FALSE, echo=TRUE, include=TRUE, warning=TRUE, results="asis">>=
## Set seed
set.seed(1)
...
Then, we run a differential network analysis and obtain the p-value \Sexpr{dn.null$ms.pval}.
<<echo=TRUE, include=TRUE, warning=TRUE, results="asis">>=
## Run diffnet (under null hypothesis)
dn.null <- diffnet_multisplit(x1,x2,b.splits=1,verbose=FALSE)
cat('p-value:',dn.null$ms.pval,'\n') # resulting p-values are in output ms.pval  
@ 

The same analysis can be performed for data generated under the
alternative hypothesis. The resulting p-value is
\Sexpr{dn.altn$ms.pval} which indicates a highly significant network
difference.
<<echo=TRUE, include=TRUE, warning=TRUE, results="asis">>=
...
dn.altn$ms.pval # p-values for all b.split data splits
plot(dn.altn) # histogram over 50 p-values
...
\texttt{ggmagsa\_multisplit} can be used to perform differential
network analyses iteratively for all gene-sets. This allows to
...
provides and implementation of differential regression. In particular,
...
For differential regression we have the option to compute p-values based on permutations.
<<echo=TRUE, include=TRUE, warning=TRUE, results="asis",fig.align='center',fig.width=3.5,fig.height=3.5>>=
...
cat('p-value: ',fit.alt.perm$medagg.pval,'\n')
@
