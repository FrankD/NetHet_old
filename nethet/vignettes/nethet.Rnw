...
\section{{\color{red}Statistical Setup and Package Functionalities}}

{\color{red} Could you add the statistical setup? Maybe something like paragraph 2 of bioinf paper?
  
  This section could introduce the statistical setup and then more or less summarize the sections which follow, i.e. the key functionalities}


The package consists of two main parts: {\color{red} I would add here
  all funcionalties: simulating, network estimation, high-dimensional
  testing, model-based clustering based on networks, plotting and exporting}
...
\texttt{nethet} that applies the \texttt{cv.glasso} function (from
package \texttt{glasso}) to each component in a heterogeneous dataset
with specified component labels.

{\color{red} the cv.glasso function is not in glasso package, I wrote
  cv.glasso. Maybe we could write that hetcvglasso is a convenient
  wrapper which performs glasso tuned with cross-validation}
